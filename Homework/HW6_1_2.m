...
for irow = nrow:-1:1
    for icol = 1:ncol
        ...
        % 1)
        % Creating the plots for each slice in the z direction
        zdirection = data(:,:,sliceNumber);
            imagesc(zdirection);
        
        % Creating the labels for the plot along the outside edge of the plot
        if(sliceNumber == 13)
            set(gca ...
                , 'XTick' , [20 40 60] ...
                , 'YTick', [10 20 30 40] ...
        ...
        % 2)
        % Using imbinarize to create a binary image of the slices
        binaryimage = imbinarize(zdirection);
            boundary = bwboundaries(binaryimage,'noholes');
        hold on;
        
        % Creating the outline of the tumor cells
